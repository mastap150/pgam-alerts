...
=============================================================
Monitors Opportunity Fill % = WINS / OPPORTUNITIES
...
Outputs:
  ...
  3. Diagnostic breakdown (only when below threshold):
       ...
       - By BUNDLE + DEMAND_ID
     Each row shows fill rate, opportunity volume, wins, and what MTD
...
MIN_OPPS_FOR_SIGNAL  = 1000     # ignore combos with fewer opportunities (too noisy)
TOP_N                = 10       # rows per diagnostic table

# ── Helpers ───────────────────────────────────────────────────────────────────

def safe_fill(wins: float, opps: float) -> float:
    """Opp Fill % = WINS / OPPORTUNITIES"""
...
def run_opp_fill_rate_agent() -> dict:
    ...
    metrics     = ["OPPORTUNITIES", "WINS", "IMPRESSIONS"]

    # 1. MTD daily rows (DATE dimension gives us both aggregate and daily)
    ...
    mtd_fill = safe_fill(mtd_wins, mtd_opps)
    alert    = mtd_fill < OPP_FILL_THRESHOLD

    # 2. Daily breakdown (reuse mtd_raw)
    daily_rows = []
    ...
    # 3. Diagnostics — only fetched when below threshold
    diag_demand     = []
    diag_pub_demand = []
    diag_bun_demand = []
...
if __name__ == "__main__":
    ...
    if result["alert"]:
        ...
        for r in result["diag_demand"][:5]:
            print(f"  {r['label']:40s}  fill={fmt_pct(r['fill_rate'])}  opps={fmt_num(r['opps'])}")
