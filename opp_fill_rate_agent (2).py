...
=============================================================
Monitors Opportunity Fill % = WINS / BIDS
...
Outputs:
  1. MTD Aggregated Opp Fill %
  2. Daily trend (current month)
  3. Diagnostic breakdown (only when below threshold):
       - By DEMAND_ID
       - By PUBLISHER + DEMAND_ID
       - By BUNDLE + DEMAND_ID
     Each row shows fill rate, bid volume, wins, and what MTD
     rate would be if that combo were removed ("If Removed →").
"""
...
MIN_BIDS_FOR_SIGNAL = 1000     # ignore combos with fewer bids (too noisy)
TOP_N               = 10       # rows per diagnostic table

# ── Helpers ───────────────────────────────────────────────────────────────────

def safe_fill(wins: float, bids: float) -> float:
    return (wins / bids) if bids > 0 else 0.0
...
def drag_impact(row_bids: float, row_wins: float,
                total_bids: float, total_wins: float) -> float:
    """How much would MTD fill rate improve if this combo were removed?"""
    new_bids = total_bids - row_bids
    new_wins = total_wins - row_wins
    return safe_fill(new_wins, new_bids) - safe_fill(total_wins, total_bids)
...
def build_diagnostic(rows: list, dim_keys: list,
                     total_bids: float, total_wins: float) -> list:
    out = []
    for r in rows:
        bids = sf(r.get("BIDS", 0))
        wins = sf(r.get("WINS", 0))
        if bids < MIN_BIDS_FOR_SIGNAL:
            continue
        fill  = safe_fill(wins, bids)
        drag  = drag_impact(bids, wins, total_bids, total_wins)
    ...
    # Worst offenders (below threshold, most drag) first
...
def _html_diag_table(title: str, icon: str, rows: list, mtd_fill: float) -> str:
    ...
    thead = "".join(
        ...
        for i, h in enumerate(["Dimension", "Bids", "Wins", "Fill Rate", "If Removed →"])
    )
    tbody = ""
    for r in rows[:TOP_N]:
        ...
        <tr style="background:{row_bg};border-bottom:1px solid {GRAY_100};">
          ...
          <td style="padding:8px;text-align:right;font-size:11px;color:{GRAY_600};">{fmt_num(r['bids'])}</td>
...
def run_opp_fill_rate_agent() -> dict:
    ...
    metrics     = ["BIDS", "WINS", "IMPRESSIONS"]

    # 1. MTD aggregate (no dimension = totals)
    mtd_raw  = fetch("DATE", metrics, month_start, today_str)
    mtd_bids = sum(sf(r.get("BIDS", 0)) for r in mtd_raw)
    mtd_wins = sum(sf(r.get("WINS", 0)) for r in mtd_raw)
    mtd_imps = sum(sf(r.get("IMPRESSIONS", 0)) for r in mtd_raw)
    mtd_fill = safe_fill(mtd_wins, mtd_bids)
    alert    = mtd_fill < OPP_FILL_THRESHOLD

    # 2. Daily breakdown (reuse mtd_raw — already has DATE dimension)
    daily_rows = []
    ...
    # 3. Diagnostics — only fetched when below threshold (saves API calls)
    diag_demand     = []
    diag_pub_demand = []
    diag_bun_demand = []
    ...
    if alert:
        # By DEMAND_ID
        raw_d = fetch("DEMAND_ID", metrics, month_start, today_str)
        diag_demand = build_diagnostic(raw_d, ["DEMAND_ID"], mtd_bids, mtd_wins)

        # By PUBLISHER + DEMAND_ID
        raw_pd = fetch("PUBLISHER,DEMAND_ID", metrics, month_start, today_str)
        diag_pub_demand = build_diagnostic(
            raw_pd, ["PUBLISHER_NAME", "DEMAND_ID"], mtd_bids, mtd_wins
        )

        # By BUNDLE + DEMAND_ID
        raw_bd = fetch("BUNDLE,DEMAND_ID", metrics, month_start, today_str)
        diag_bun_demand = build_diagnostic(
            raw_bd, ["BUNDLE", "DEMAND_ID"], mtd_bids, mtd_wins
        )
    ...
    for r in daily_rows:
        ...
        <tr style="background:{bg};border-bottom:1px solid {GRAY_100};">
          ...
          <td style="padding:6px 10px;text-align:right;font-size:11px;color:{GRAY_600};">{fmt_num(r['bids'])}</td>
    ...
    if alert:
        ...
        <div style="margin-top:28px;">
          ...
          <p style="font-size:12px;color:{GRAY_600};margin:0 0 20px 0;">
            Combos with ≥{fmt_num(MIN_BIDS_FOR_SIGNAL)} bids only.
...
<div style="font-family:Arial,sans-serif;margin:24px 0;">
  ...
  <p style="font-size:12px;color:#666;margin:0 0 16px 0;">
    Threshold: ≥ 0.05% (WINS ÷ BIDS) — must hold for full month to avoid fee.
  </p>
  {alert_banner}
  <div style="background:{mtd_bg};border:1px solid {mtd_border};border-radius:8px;
    ...
    <div style="font-size:12px;color:#555;">
      ...
      {fmt_num(mtd_bids)} bids &nbsp;|&nbsp; {fmt_num(mtd_imps)} impressions
    </div>
  </div>
    ...
    <table style="border-collapse:collapse;width:100%;font-size:12px;">
      <thead>
        <tr style="background:{NAVY};color:white;">
          ...
          <th style="padding:8px 10px;text-align:right;">Bids</th>
...
if __name__ == "__main__":
    ...
    print(f"MTD Bids      : {fmt_num(result['mtd_bids'])}")
    print(f"MTD Wins      : {fmt_num(result['mtd_wins'])}")
    if result["alert"]:
        print(f"\nTop Demand drag ({len(result['diag_demand'])} combos):")
        for r in result["diag_demand"][:5]:
            print(f"  {r['label']:40s}  fill={fmt_pct(r['fill_rate'])}  bids={fmt_num(r['bids'])}")
